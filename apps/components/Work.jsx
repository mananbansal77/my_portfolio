...
const Work = ({isDarkMode}) => {
  return (
    ...
    transition={{delay:0.3,duration:1}} className='text-center mb-2 textlg font-Ovo'>
        My Portfolio
      </motion.h4>
      <motion.h2
      initial={{y:-20,opacity:0}}
    ...
    transition={{delay:0.7,duration:0.5}} className='text-center max-w-2xl mx-auto mt-5 mb-12 font-Ovo'>
              difvbsubvksjdjsdbhsd sjbvjhsbvjd sdbhsdjsbd sdusbdjs

        </motion.p>
        <motion.div 
        initial={{opacity:0}}
    whileInView={{opacity:1}}
    transition={{delay:0.9,duration:0.6}}className='grid grid-cols-4 my-10 gap-5 dark:text-black'>
